...
async def test_e2e_complete_briefing_flow_from_authorized_phone(
    ...
    # Create organization with WhatsApp settings
    ...
    # Create architect and template in the organization
    test_architect = await ArchitectFactory.create_async(organization=test_organization)
    project_type = await ProjectTypeFactory.create_async(residencial=True)
    test_template = await BriefingTemplateFactory.create_with_version_async(
        organization_template=True,
        organization=test_organization,
        created_by=test_architect,
...
async def test_e2e_extraction_failure_sends_error_to_architect(
    ...
    # Create architect in the organization
    test_architect = await ArchitectFactory.create_async(organization=test_organization)
...
async def test_e2e_duplicate_briefing_resumes_existing(
    ...
    # Create existing client with in-progress briefing
    existing_client = await EndClientFactory.create_async(
        name="João Silva",
        phone="+5511999888777",
        organization=test_organization,
