...
@pytest.fixture
async def template_with_mixed_questions(db_session: AsyncSession, test_project_type):
...
async def briefing_with_mixed_template(
    ...
    briefing = await BriefingFactory.create_async(
        ...
        template_version=template_with_mixed_questions,
...
@pytest.fixture
async def template_with_multiple_choice(db_session: AsyncSession, test_project_type):
...
async def briefing_with_multiple_choice(
    db_session: AsyncSession,
    test_end_client,
    template_with_multiple_choice,
):
    """Create briefing with multiple choice template."""
    briefing = await BriefingFactory.create_async(
        end_client=test_end_client,
        template_version=template_with_multiple_choice,
        current_question_order=1,
        answers={},
        status="IN_PROGRESS",
    )
