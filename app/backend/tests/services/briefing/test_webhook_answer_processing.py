...
@pytest.fixture
async def test_org_with_whatsapp(db_session: AsyncSession):
...
@pytest.fixture
async def test_client(db_session: AsyncSession, test_org_with_whatsapp, test_architect):
    """Create test end client."""
    return await EndClientFactory.create_async(
        organization=test_org_with_whatsapp,
...
@pytest.fixture
async def active_briefing(db_session: AsyncSession, test_client, test_template):
...
@pytest.fixture
async def whatsapp_session(db_session: AsyncSession, test_client):
    """Create WhatsApp session for client."""
    return await WhatsAppSessionFactory.create_async(
        end_client=test_client,
...
async def test_session_index_updated_after_answer_e2e(
    ...
    """
    # Create test client
    test_client = await EndClientFactory.create_async(
        organization=test_org_with_whatsapp,
        architect=test_architect,
        name="Test Client",
        phone="+5511987654321",
    ...
    active_briefing = await BriefingFactory.create_async(
        end_client=test_client,
        template_version=test_template.current_version,
        status=BriefingStatus.IN_PROGRESS,
        current_question_order=1,
    ...
    whatsapp_session = await WhatsAppSessionFactory.create_async(
        end_client=test_client,
...
async def test_session_index_resets_when_starting_new_briefing_after_completion(
    ...
    first_briefing = await BriefingFactory.create_async(
        end_client=test_client,
        template_version=test_template.current_version,
    ...
    second_briefing = await BriefingFactory.create_async(
        end_client=test_client,
        template_version=test_template.current_version,
        status=BriefingStatus.IN_PROGRESS,
        current_question_order=1,
